...
/**
 * Member activate servlet - activates a member account.
 */
@WebServlet(name = "MemberActivateServlet", urlPatterns = {"/librarian/members/activate"})
public class MemberActivateServlet extends HttpServlet {

    private final MemberManagementService memberService;

    public MemberActivateServlet() {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Activate member
            memberService.activateMember(memberId, currentUser.getUserId(), ipAddress);

            SessionUtil.setSuccessMessage(session, "Member activated successfully");
            response.sendRedirect(request.getContextPath() + "/librarian/members");
            ...
            SessionUtil.setErrorMessage(session, "Error activating member: " + e.getMessage());
            response.sendRedirect(request.getContextPath() + "/librarian/members");
        }
    }
}
